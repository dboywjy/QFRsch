...
- 因子基类设计：所有因子必须继承z指定因子类，强制实现 `compute()` 方法
...
- 参考使用backltrader/bt作为回测层的回测引擎，backtrader是比较多用户使用的回测平台
...
```mermaid
gantt
    title BackTesting4all
    dateFormat  YYYY-MM-DD
    axisFormat  %m-%d

    section 数据层开发
    架构与技术选型           :done,    des1, 2026-01-08, 3d
    数据对齐与日历对齐       :active,  d2,   2026-01-12, 7d

    section 因子层
    因子基类设计             :crit,    f1,   after d2, 3d
    公式化算子库             :         f2,   after f1, 2d
    流式计算与存储           :         f3,   after f2, 2d
    因子层完成里程碑         :milestone, m2, after f3, 0d

    section 策略与回测
    策略适配（库集成）       :         b1,   after f3, 7d

    section 模型策略层
    模型/ML 管道开发         :         a1,   after b1, 7d
    ...
    section 报表与验收
    Tearsheet 报表 (HTML)    :         r1,   after an1, 5d
    验收（最终验收）         :         t1,   after r1, 7d
    文档与最终演示           :         end1, after t1, 3d
```
